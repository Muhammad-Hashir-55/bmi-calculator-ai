...
export function BmiGauge({ bmi }: BmiGaugeProps) {
    ...
    return (
        <div className="w-full py-6 flex flex-col items-center gap-4">
            {/* Gauge Scale */}
            <div className="relative w-full h-4 rounded-full overflow-hidden bg-white/10 flex">
            ...
            <div className="relative w-full h-1 mt-1">
                {bmi !== null && (
                    <motion.div
                        ...
                        className="absolute -top-6 -ml-3 w-6 h-6 flex items-center justify-center filter drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]"
                    >
                        <div className="w-0 h-0 border-l-[8px] border-l-transparent border-t-[12px] border-t-white border-r-[8px] border-r-transparent" />
                    </motion.div>
                )}
            </div>
            ...
            <div className="text-center mt-2 h-[80px]">
                {bmi ? (
                    <motion.div key={bmi} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                        <h2 className="text-5xl font-black bg-clip-text text-transparent bg-gradient-to-br from-white to-white/60 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
                            {bmi.toFixed(1)}
                        </h2>
                    </motion.div>
                ) : (
                    <h2 className="text-5xl font-black text-white/20">--</h2>
                )}
